...
public class MoCProxy implements IGuiHandler {
    ...
    public boolean useHumanModelAndMinecraftComesAliveVillagerTexturesForWitcheryWerewolfEntities;
    ...
    public void readGlobalConfigValues() 
    {
        ...
        useHumanModelAndMinecraftComesAliveVillagerTexturesForWitcheryWerewolfEntities = mocSettingsConfig.get(CATEGORY_MOC_MOD_INTEGRATION_SETTINGS, "useHumanModelAndMinecraftComesAliveVillagerTexturesForWitcheryWerewolf", true, "Only has an effect if replaceWitcheryWerewolfEntities is true and both the Witchery mod and Minecraft Comes Alive mod are installed. Uses human model and Minecraft Comes Alive textures for the human forms of Witchery werewolf entities.").getBoolean(true);
        replaceWitcheryPlayerWolf = mocSettingsConfig.get(CATEGORY_MOC_MOD_INTEGRATION_SETTINGS, "replaceWitcheryPlayerWolf", true, "All players must have this enabled if the server they are playing on has it enabled - if a player has this disabled while the server has it enabled, that player will experience visual glitches. THIS IS NOT COMPATIBLE WITH ANY OTHER MODS THAT CAN CHANGE THE PLAYER'S MAX HEALTH. Only has an effect if the Witchery mod is installed. Replaces the Witchery player wolf model with the Mo' Creatures dire wolf model for player werewolves that are level 7 and above. Werewolf and dire wolf respect will not work for level 7+ player wolf form if this is false.").getBoolean(true);
