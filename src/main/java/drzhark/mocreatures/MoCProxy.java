...
public class MoCProxy implements IGuiHandler {
    ...
    public boolean generateWolfAltarReplacementMod;
    ...
    public void readGlobalConfigValues() 
    {
        ...
        enableRareGiantPandaVariant = mocSettingsConfig.get(CATEGORY_MOC_CREATURE_GENERAL_SETTINGS, "enableRareGiantPandaVariant", false, "If true: pandas that spawn will have a 10% chance to be a giant panda.").getBoolean(false);
        ...
        rareItemDropChance = mocSettingsConfig.get(CATEGORY_MOC_CREATURE_GENERAL_SETTINGS, "rareItemDropChance", 25, "A value of 25 means Horses, Ostriches, Scorpions, etc. will have a 25% chance to drop a rare item such as a heart of darkness, unicorn horn, ect. when killed. Raise the value if you want higher drop rates.").getInt();
        ...
        generateWolfAltarReplacementMod = mocSettingsConfig.get(CATEGORY_MOC_MOD_INTEGRATION_SETTINGS, "generateWolfAltarReplacementMod", false, "Works on the client side and only has an effect if the Witchery mod is installed. Generates the Wolf Altar Replacement mod in the mods folder. This replaces the model and texture of the Wolf Altar from the Wichery mod to resemble the statue of a direwolf. After enabling this, it will take two game launches to take effect. The first launch is to generate the mod only, the second launch is where the mod is recognized for mod loading. This configuration setting will automatically set itself to false after it has generated the mod file in the mods folder.").getBoolean(false);
